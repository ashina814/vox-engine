...
def test_pipeline_metadata_fields(pipeline):
    ...
    res = pipeline(InferenceRequest(input_wav=wav, num_diffusion_steps=2, target_key=None))
    assert "elapsed_s" in res.metadata
    assert "rtf" in res.metadata
    assert res.metadata["num_diffusion_steps"] == 2
...
def test_pipeline_loads_wav_file(tmp_path, pipeline):
    ...
    res = pipeline(InferenceRequest(input_wav=path, num_diffusion_steps=2, target_key=None))
    assert res.output_wav.shape[0] > 0
